  ...
  <div class="preview-page">
    ...
    <template v-for="(item, taskIndex) in material.content">
      ...
      <template v-if="item.type=='task' && taskIndex<=maxIndex && taskIndex>minIndex">
        ...
        <p class="push_btn" v-if="isPushed" @click="linkTo(`/agency_tour/course_control/${id}/${index + 1}`)">下一页</p>
      ...
      <div v-if="item.type=='task' && taskIndex<maxIndex && taskIndex>=minIndex"> <!-- 任务卡详情 -->
        <template v-for="(question, questionindex) in item.questions">
          <ModuleWrap v-if="question.type==='photo'" @click.native="linkTo('/agency_tour/course_answer/' + id + '/' + taskIndex + '/' + questionindex)">
          ...
          <ModuleWrap v-if="question.type==='choose' || question.type==='mulchoose'" @click.native="linkTo('/agency_tour/course_answer/' + id + '/' + taskIndex + '/' + questionindex)">
            <p slot="header" class="center">{{ `${item.title} 任务${num2CharMap[questionindex+1]}` }}</p>
            <div slot="body" class="module_body">
              <p class="question"  v-if="question.question" v-html="`${questionindex + 1}.[${taskTypeMap[question.type]}]` + question.question.replace(/\n/g, '<br />')"></p>
          ...
          <ModuleWrap v-if="question.type==='sound'" @click.native="linkTo('/agency_tour/course_answer/' + id + '/' + taskIndex + '/' + questionindex)">
            <p slot="header" class="center">{{ `${item.title} 任务${num2CharMap[questionindex+1]}` }}</p>
            <div slot="body" class="module_body">
              <p class="question"  v-if="question.question" v-html="`${questionindex + 1}.[${taskTypeMap[question.type]}]` + question.question.replace(/\n/g, '<br />')"></p>
              <div class="idea" v-if="question.prompt" v-html="'提示：' + question.prompt.replace(/\n/g, '<br />')"></div>
      ...
      <template v-if="maxIndex === 999 && taskIndex==material.content.length-1 && hasHomework">
        ...
        <p class="push_btn" v-if="isHomeworkPushed" @click="linkTo('/agency_tour/comment/' + id)">写点评</p>
        <p style="padding:10px;"></p>
      </template>
...
<script>
  ...
  export default {
    ...
    beforeRouteLeave (to, from, next) {
      if (to.path.indexOf('/agency_tour/comment/') !== -1 || to.path.indexOf('/agency_tour/course_answer/') !== -1) {
        next()
        return
      }
      this.$vux.confirm.show({
        title: '确认退出上课流程？',
        content: this.$t('下次进入课程将不显示已推送的任务和内容'),
        confirmText: '确认',
        onConfirm () {
          next()
        },
...
//    var toIndex = to.path.split('/')[to.path.split('/').length - 1]
//    var fromIndex = from.path.split('/')[from.path.split('/').length - 1]
    ...
    methods: {
      ...
      pushTask (index, type, title, linkTo = true) {
        this.showConfirm('是否推送' + title + '？', () => {
          switch (type) {
            case 'task':
              this.$api.courseTour.pushTask(this.id, index).then((res) => {
                this.$vux.toast.show({
                  text: '推送成功',
                  type: 'success'
                })
                if (linkTo) this.$router.push(`/agency_tour/course_control/${this.id}/${this.index + 1}`)
              })
              break
            case 'summary':
              this.$api.courseTour.pushTaskSummary(this.id, index).then((res) => {
                this.$vux.toast.show({
                  text: '推送成功',
                  type: 'success'
                })
      ...
      endClass() {
        this.$api.course.classEnd(this.id).then(res => {
          this.$router.push('/agency_tour/comment/' + this.id)
        }).catch(() => {
          this.$router.push('/agency_tour/comment/' + this.id)
        })
      }
