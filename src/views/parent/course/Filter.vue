...
export default {
  ...
  created () {
    ...
    // this.$api.label.play_count().then((data) => {
    //   var playCount = data.list
    //   var list = []
    //   for (var type in playCount) {
    //     if (playCount.hasOwnProperty(type)) {
    //       playCount[type].forEach((item) => {
  ...
  methods: {
    ...
    search () {
      ...
      this.$router.push('/parent/course/list/' + filterStr)
    },
    select (label) {
