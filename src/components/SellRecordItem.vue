<template>
  <div :class="[record.status == '2' ? 'bggreen' : '', 'cl-module']">
    <div class="cl-m-body">
      ...
      <template v-if="record.status==1 && record.payType==1">
        <hr>
        <router-link :to="'/seller/sign_up/' + record.id">
          <div class="bottom">
...
export default {
  ...
  data () {
    return {
      ...
      textList: ['家长姓名', '家长手机', '商品名称', '商品价格', '订单编号']
    }
  },
  props: {
    record: {}
  ...
  computed: {
    dataList() {
      return [this.record.name, this.record.phone, this.record.title, this.record.price, this.record.orderId]
    }
  },
  methods: {
    gotoDetail () {
      this.$router.push('/seller/record_detail/' + this.record.id)
    },
    statusLabel (record) {
      if (record.payType === 1) {
