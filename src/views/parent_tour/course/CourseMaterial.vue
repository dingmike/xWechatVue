        ...
        <div class="cl-module">
          ...
          <div class="cl-m-body">
            ...
            <div class="port">
              ...
              <span slot="icon" class="iconfont icon-plus add-photo" @click="uploadImg(index)" v-if="photoNum < 9 && !hasAnswer"></span>
              <div class="clear"></div>
            </div>
    ...
    methods: {
      init() {
        ...
        this.$axios.all([this.getThirdPartyList()])
          .then(() => {
            this.id = id
            this.$api.courseTour.reportClick(id).then((course) => {
      ...
      uploadImg(index) {
        ...
        this.$wechat.chooseImage({
          ...
          success: (res) => {
            ...
            this.forUploadImg(0, localIds, index)
//
...
//          })
      ...
      forUploadImg (index, urlList, tIndex) {
        ...
        this.$wechat.uploadImage({
          ...
          success: (res) => {
            ...
            var serverId = res.serverId
            this.$api.photo.uploadImg(serverId).then((data) => {
              this.answerList[tIndex].push(data.url)
              index++
              this.forUploadImg(index, urlList, tIndex)
            })
            .catch(e => {
              this.photoNum -= 1
            })
      ...
      uploadAnswer () {
        ...
        this.showConfirm('确定要提交答案？', () => {
          ...
          this.$api.courseTour.setTaskAnswer(this.id, this.methodindex, this.answerList).then((res) => {
      ...
      setShare() {
        var title = `快看,我正和孩子参加「${this.thirdPartyList[this.course.userId].name}」的陪伴技巧体验营「${this.course.title}」`
