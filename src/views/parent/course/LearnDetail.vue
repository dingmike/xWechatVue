      ...
      <x-swipe-item v-if="0">
        <div>
          <div class="cl-module">
            ...
            <div class="cl-m-body">
              <div class="public" v-html="learn.content.replace(/\n/g, '<br />')">
              </div>
            </div>
          </div>
        </div>
          ...
          <div>
            <div class="cl-module">
              ...
              <div class="cl-m-body">
                ...
                <div class="public">
                  <x-textarea class="xj-textarea" :placeholder="ask.sug" v-model="ask.answer.content"></x-textarea>
                </div>
                <div class="port">
                  <span slot="icon" class="iconfont icon-plus add-photo" @click="uploadImg(ask)"></span>
                  <span class="preview">
                    <img v-if="ask.answer.photo" :src="ask.answer.photo">
                  </span>
                  <div class="clear"></div>
                </div>
                <x-button type="primary" @click.native="saveAnswer(ask)">保存</x-button>
              </div>
            </div>
          </div>
        </x-swipe-item>
        ...
        <div>
          <div class="cl-module">
            <div class="cl-m-body finish">
              ...
              <div>
                <p>感谢参与心迹家庭成长社课程</p>
                <p>请点击<strong>提交</strong>并等待老师总结</p>
              </div>
...
export default {
  ...
  methods: {
    ...
    submit () {
      var answerList = []
      this.learn.tasks.forEach((task) => {
        task.asks.forEach((item) => {
          var answer = item.answer
          answer.ask_id = item.id
