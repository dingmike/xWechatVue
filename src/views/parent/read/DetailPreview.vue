  ...
  methods: {
    init () {
      ...
      this.$label.getLabelList().then((list) => {
        this.$api.reader.getPreviewDetail(id).then((data) => {
    ...
    setShare () {
      var option = {
        ...
        desc: this.reader.description,
        imgUrl: window.location.origin + '/static/img/logo.png'
      }
      if (this.reader.cover) {
        option.imgUrl = window.location.origin + window.location.pathname + this.reader.cover.replace('["./', '')
      }
