    ...
    <!--<div class="cl-module">-->
      <div class="cl-m-body">
        <div class="public">
          ...
          <p class="share-info">前两天和孩子参加「{{thirdPartyList[course.userId].name}}」的陪伴技巧体验坊「{{ course.title }}」，跟小家伙一起完成老师布置的课后作业，有意思~ </p>
          <p class="share-icon">︾</p>
        </div>
      </div>
    <!--</div>-->
    <!--<template v-for="(task, index) in material.taskList">-->
    <template v-for="(question, index) in task.question">
      <template v-if="question.type==='photo'">
        <div class="cl-module">
          <div class="cl-m-header">
            {{ task.title }} {{thirdPartyId == 1 ? '任务': '题目'}}{{ num2ChineseMap[index+1] }}
          </div>
          <div class="cl-m-body">
          ...
          <div class="cl-m-body">
            ...
            <hr>
              <div class="public flexset vux-flex-row">
                ...
                <button class="voice-button" @touchstart.prevent="startRecord(index, $event)">
  ...
  export default {
    ...
    created() {
      ...
      this.$axios.all([this.getThirdPartyList()])
        .then(() => {
          this.$api.course.getShareCourseSummary(this.userId, id)
            .then((course) => {
              ...
              this.task = course.material.homework
